...
const PostPreview = ({ subtitle, title, date, counter = 100, slug, lang }) => {
  ...
  return (
    <div className="flex flex-col w-full justify-between overflow-hidden  bg-zinc-800 border rounded-md shadow-sm  min-w-[350px] max-w-[350px] sm:min-w-[300px] sm:max-w-[300px] md:min-w-[230px] md:max-w-[250px] border-slate-300 ">
...
function Subtitle(props) {
  return (
    <p className="p-4 text-sm text-slate-200 line-clamp-1 hover:line-clamp-none">
      {props.subtitle}
    </p>
  );
}
  ...
  return (
    <div className="flex items-center justify-between p-2 transition-all transform bg-zinc-700/50 hover:scale-110">
      <Link href={`/blog/${props.slug}`}>
        <h2 className="text-center text-white text-md hover:underline">
          {props.title}
        </h2>
      </Link>
    </div>
